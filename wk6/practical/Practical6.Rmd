...
With the `glm()` function you can specify a logistic regression model. You can save a model in object and request summary statistics with the `summary()` command. When a model is stored in an object you can ask for the coefficients with `coefficients()`. 
...
```{r, include=params$answers}
# In the case of a continuous predictor there is no real reference group. Instead, the intercept is the log-odds of survival when age = 0. In this model, the log-odds of survival for passengers of age 0 is 0.019.
```
...
# For every increase in age of one year, the odds of survival are 0.99 times the odds of those with one age unit less, or -1.09%.
```
...
# For model 1, the null deviance is 964.52 and the residual deviance is 960.23 For model 2, the null deviance is 964.52 and the residual deviance is 635.39
```
...
# The analysis of deviance indicates that there is a reduction in residual deviance of 324.84 that is statistically significant. Model 2 is a better model. For a binomial model, the statistical test should be the chi-square difference test.
```
...
You are now at the end of this week's practical. Next week, we will spend some time on model fit, comparison, and assumptions. 
