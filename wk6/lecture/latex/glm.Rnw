...
\begin{frame}{Defining the Multinomial Logistic Regression Model}
  ...
  \begin{itemize}
  \item The random component in our multinomial logistic regression model will
    be the multinomial distribution.
  \end{itemize}
  \vb
...
\begin{frame}{Full Multinomial Logistic Regression Model}
  ...
  regression model is:
  \begin{align*}
    Y &\sim \mathrm{Multinom}(\Pi, \mathbf{1}), ~~ \Pi = \{\pi_2, \pi_3, \ldots,
    \pi_L\}\\
    \mathrm{logit}(\pi_l) &= \beta_{l0} + \sum_{p = 1}^P \beta_{lp} X_p, ~~ l = 2, 3,
    \ldots, L
  \end{align*}
  The fitted model can be represented as:
  \begin{align*}
    \mathrm{logit}(\hat{\pi}_l) = \hat{\beta}_{l0} + \sum_{p = 1}^P \hat{\beta}_{lp}
    X_p, ~~ l = 2, 3, \ldots, L
  \end{align*}
...
\item By fitting this model to our usual ``diabetes'' data we get:
  \begin{align*}
    ...
    \mathrm{logit}(\hat{\pi}_{mid.gluc}) &=
...
\begin{frame}{Predictions from Multinomial Logistic Regression}
  ...
  \item The only difference is that the multinomial logistic regression model
    ...
    \begin{align*}
      \hat{\eta}_l' &=
      \begin{cases}
        \hat{\beta}_{l0} + \sum_{p = 1}^P \hat{\beta}_{lp} X_p' & \mathrm{if $l > 1$}\\
        0 & \mathrm{if $l = 1$}
      \end{cases}\\[8pt]
...
\begin{frame}{Predictions from Multinomial Logistic Regression}
  ...
  \begin{align*}
    \mathrm{softmax}(X_l) = \frac{e^{X_l}}{\sum_{j = 1}^Le^{X_j}}
  \end{align*}
...
\begin{tabular}{rllll}
  $\mathrm{logit}\left(\frac{\pi_{low.gluc}}{\pi_{low.gluc}}\right)$ &=&&&\\
  \Sexpr{b10} &+ \Sexpr{b11} (57 - 40) &+ \Sexpr{b12} (28 - 25) &+ \Sexpr{b13} (92 - 100) &=
  \Sexpr{eta2[1]}\\[8pt]
  $\mathrm{logit}\left(\frac{\pi_{mid.gluc}}{\pi_{low.gluc}}\right)$ &=&&&\\
  \Sexpr{b20} &+ \Sexpr{b21} (57 - 40) &+ \Sexpr{b22} (28 - 25) &+ \Sexpr{b23} (92 - 100) &=
  \Sexpr{eta2[2]}\\[8pt]
  $\mathrm{logit}\left(\frac{\pi_{hi.gluc}}{\pi_{low.gluc}}\right)$ &=&&&\\
...
\begin{frame}[fragile]{Confusion Matrix}
  ...
  One of the most direct ways to evaluate classification performance
  is to tabulate the true and predicted classes.

<<echo = FALSE, results = "asis">>=
tab <- table(True = diabetes$highGlu,
             Predicted = as.numeric(predict(out1, type = "response") > 0.5)
             )
xtable(tab, caption = "Confusion Matrix")
@
