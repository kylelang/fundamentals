...
needed to successfully complete the assignments. The exercises will be graded on 
a completion basis. 

- You must finish the `R` exercises for 6 out of 7 weeks to pass the course.

To pass the course:

1. Your final grade must be 5.5 or higher
1. You must pass both assignments
1. You must submit `R` exercises for at least 6 of the first 7 weeks

## Column 2 {data-width=400}

### Schedule

| Week # | Topic | `R`-Exercise | Workgroup | 
|-|---|----|----|
| 1 | The elemental building blocks of `R` | Assigning values to objects; Creating vectors, matrices, data frames, and lists | Receive instructions and form groups |
| 2 | Data manipulation; Least squares | Data manipulation; Using pipes to simplify workflows | Locate a data set for predictive modeling and formulate a research hypothesis (Make sure the data can support both linear and logistic regression) |
| 3 | Linear modeling 1; Data visualization | The `lm()` function in `R`; Visualizing bivariate relations | Specify a linear model; Fit your defined model |
| 4 | Linear modeling 2; Assumptions, diagnostics | Investigating the assumptions of the linear model | Check the assumptions of your model; Use your model to test your hypotheses; Continue the project in `rmarkdown` |
...
| 6 | Generalized linear model; Logistic regression 1 | The `glm()` function in `R`; modeling, prediction, and visualization | Fit your defined model; evaluate if the assumptions are met |
| 7 | Logistic regression 2 | Finish exercise from last week | Test and quantify the effect of the defined model |
...
[SurfDrive][sd] drop folder. Name the file `your_name.R`. Where `your_name` is 
your full name in *lower snake-case*.

- E.g., My name is Kyle Lang, so my submission would be called `kyle_lang.R`.
...
You can find the lecture slides [here](wk2/lecture2.html)

### Workgroup (Thursday)

In today's workgroup, you will locate a data set for predictive modeling and 
...
[SurfDrive][sd] folder. Name the files `your_name.Rmd` and `your_name.html`, 
respectively. Where `your_name` is your full name in *lower snake-case*. 
...
and its assumptions.
1. Data Visualization with **ggplot2**: The **ggplot2** package is one of the 
...
You can find the slides on linear modeling [here]()

Rather than re-inventing the wheel, we will refer you to some existing resources for the ggplot introduction. 

<iframe width="560" height="315" src="https://www.youtube.com/embed/N5gYo43oLE8" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

<!--
![ggplot tutorial](https://www.youtube.com/watch?v=N5gYo43oLE8&list=PLYiJ7zIgJfiMoR_EY9xsZ0IFd7LqFSHuM&index=1)
-->
