---
title: "Fundamental Techniques of Data Science - R Practical Week 5"
...
library(foreign) # for data sets in other file formats (such as .spss or .sas)
library(regclass)
library(MASS) # for some data sets
```
...
<li> Species of the fish
<li> Weight of the fish in grams
<li> Vertical, length of the fish in cm
<li> Diagonal length of the fish in cm
<li> Cross length of the fish in cm
<li> Height of the fish in cm
<li> Diagonal width of the fish in cm </li>
<br>
...
**1.a. Check whether there is a linear relation between the variables vertical length and the cross length.**
```{r}
ggplot(data_fish, 
...
**1.b.Next check the relation between weight and height.**
```{r}
...
**1.c. Describe both plots. What differences do you see?**
```{asis}
...
**2.a. Apply a log-transformation to the weight variable.**
```{r}
data_fish$weigth_trans <- data_fish$weigth %>% 
  log()
```

**2.b. Plot the relation between length and weight again, but now including the transformed variable.** 
```{r}
...
**3.a. Specify a linear model with `weight` as outcome variable using all other variables in the dataset as predictors. Save this model as `model_fish1`. Calculate VIF values for this model.**
```{r}
...
**3.b. Check the VIF scores. If VIF scores exceed a score of 10, give substantial explanation why the VIF scores are this high.**
```{asis}
...
**3.c.What adjustments can be made to the model to account for multicollinearity in this case?**
```{asis}
...
**3.d. Run a new model which only includes one of the three length variables and call it `model_fish2`. Describe if there is an improvement.**
```{r}
...
**4. What happens with the regression model when there are more predictors than observations?**
...
**5. What is the possible consequence of not meeting this assumption?**
...
**6.a. Create a residual vs fitted values plot for `model_fish1`, which is the first plot generated by the `plot()` function.** 
```{r}
model_fish1 %>%
  plot(1)
```

**6.b. Load in the `iris` data, and specify a model where sepal length is predicted by all other variables and save this as `model_iris1`.**
```{r}
data_iris   <- iris
...
**6.c. Create a residual vs fitted plot for this model as well.** 
```{r}
model_iris1 %>%
  plot(1)
```

**6.d. Discuss both plots and indicate whether the assumption is met.**
```{asis}
...
**6.e. Discuss what the consequence would be if this assumption is violated.**
```{asis}
...
**7. How can both causes of correlated error terms be detected, and what can be done to solve the problem?**
...
**8. Create a QQ plot for `model_iris1`, which is the second plot generated by the `plot()` function. Indicate whether the assumption is met.**
...
**8b. Create a new model using the fish data, where `diagonal_width` is predicted by `cross_length`, and store the model as `model_fish3`.**
```{r}
model_fish3 <- lm(diagonal_width ~ cross_length, 
                  data = data_fish)
```

**8.c. Create a QQ plot for `model_fish3`.**
```{r}
model_fish3 %>%
  plot(2)
```

**8.d. Interpret the two plots. Is the assumption met in both cases?**
```{asis}
...
**9. In what cases is it problematic that the assumption is not met? And in what cases is it no problem?**
...
**10.a. Make a plot of studentized residuals by using the functions `rstudent` and `plot` for `model_fish1. What do you conclude?**
```{r}
model_fish1 %>%
  rstudent() %>%
  plot()
...
**10.b. Make a plot of studentized residuals for `model_iris1`.**
```{r}
model_iris1 %>%
  rstudent() %>%
  plot()
...
**10.c. Store the dataset `Animals` from the `MASS` package. Define a regression model where animals' body weight is predicted by brain weight and store it as `model_animals1`.**
```{r}
...
**10.d. Make a plot of the studentized residuals for `model_animals1`.**
```{r}
model_animals1 %>%
  rstudent() %>%
  plot()
...
**11. For the model specified under model_animals1, create a leverage plot by plotting the `hatvalues()` of the model.**
...
**12.a. For `model_animals1`, check Cooks distance by plotting the `cooks.distance` of the model.**
```{r}
model_animals1 %>%
  cooks.distance() %>%
  plot()
```
**12.b. For `model_animals1`, check the DFBETAS by using the function `dfbetas`.**
```{r}
...
**12.c. Describe what you see in the plots for Cook's distance and DFBETAS. What do you conclude?**
...
**13.a. Delete the problematic observation that you found in Question 12 and store the dataset under a new name.**
```{r}
data_animals2 <- data_animals[-26,]
```

**13.b. Fit the regression model where animals' body weight is predicted by brain weight using the adjusted dataset and store it as `model_animals2`.**
```{r}
model_animals2 <- lm(body ~ brain, 
                     data = data_animals2)
```

**13.c. Compare the output to `model_animals1` and describe the changes.**
```{r}
...
**13.d. Run the plots for influential observations again on this new model and see if anything changes.**
