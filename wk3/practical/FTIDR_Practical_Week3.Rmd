...
```
```{r}
ggarrange(p1, p2, p3, nrow = 2, ncol = 2, vjust = 0.5, common.legend = TRUE, legend = "bottom")
```

## Writing functions

When doing EDA, you might find that you repeat the same operations again and again on different variables. In some cases, writing your own function can reduce this repetition. For example, the function below checks the class of each variable in the dataframe and calculates the mean and standard deviation for numeric/integer and counts the levels for factors. 

```{r, eval = FALSE}
stats <- function(df){
  
  # to store output
  out_list <-  vector("list", num_cols)
  names(out_list) <- names(df)
  
  # loop over columns
  for(i in 1:ncol(df)){
    
    var <- df %>% pull(names(df)[i])
    
    # make table if a factor
    if(is.factor(var)){
      out_list[[i]] <- table(var)
    } else {
      # get mean and sd if numeric or integer
      if(is.numeric(var) | is.integer(var)){
        out_list[[i]] <- data.frame(mean = mean(var), sd = sd(var))
      }
    }
  }
