...
p1 <- ggplot(dat1, aes(x = B, y = density)) + theme_classic()
p1 + geom_line() +
...
\begin{frame}{Test Statistics}
  ...
  We can use $SE\left(\hat{\beta}\right)$, in combination with $\hat{\beta}$, 
  for inference.
  \begin{itemize}
  \item We can construct a Wald statistic for $\hat{\beta}$ as follows:
  \end{itemize}
  \begin{align*}
...
se12 <- round(se1, 2)
df <- nrow(Cars93) - 2
moe  <- round(qt(0.975, df = df) * se12, 2)
@

For the slope in our example problem, we get a Wald statistic of:
\begin{align*}
  t = \frac{\hat{\beta}_1}{SE\left(\hat{\beta}_1\right)} = 
    \frac{\Sexpr{b1}}{\Sexpr{se12}} = \Sexpr{b1 / se12}
\end{align*}
So, it is highly unlikely that these data originate from a population wherein
car price and horsepower are not linearly related.
...
\begin{frame}{Inferential Tools: Confidence Intervals}
  ...
  \begin{align*}
    ...
    \Sexpr{se12} = [\Sexpr{b1 - moe}; \Sexpr{b1 + moe}]
  \end{align*}
...
%------------------------------------------------------------------------------%
\comment{%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[shrink = 5]{Inference Example}
...
For our example problem, we get a Wald statistic of:
\begin{align*}
  t = \frac{\Sexpr{b1}}{\Sexpr{se12}} = \Sexpr{b1 / se12}
\end{align*}
So, it is highly unlikely that these data originate from a population wherein
car price and horsepower are not linearly related.\\
\va
Furthermore, we get a 95\% CI of:
\begin{align*}
  CI_{95} = \Sexpr{b1} \pm \Sexpr{round(qt(0.975, df = df), 2)} \times
  \Sexpr{se12} = [\Sexpr{b1 - moe}; \Sexpr{b1 + moe}]
\end{align*}
...
\vb
\begin{itemize}
\item We are \emph{95\% certain} in the sense that if we repeat this analysis an
  infinite number of times, 95\% of the CIs that we calculate will surround
  the true value of $\beta_1$.
\end{itemize}
