...
\begin{frame}[fragile, allowframebreaks]{Example}
  ...
  We can implement the internally studentized residual method via the
  \src{scale()} function.
  ...
  We can implement the externally studentized residual method by applying the
  \src{rstudent()} function to a fitted intercept-only model.
  
  <<>>=
  ## Compute the absolute studentized residuals:
  z <- lm(bmi ~ 1, data = boys, na.action =  "na.exclude") %>%
      rstudent() %>%
      abs()
  ...
  We can implement the boxplot method via the \src{boxplot.stats()} function.
