...
# False positives (top right) - predicted people survived, when they did not
# False negatives (bottom left) - predicting people did not survived, but they did
...
# Model sensitivity is 0.92, meaning that if the passenger did not survive, there is a 92% chance the model will detect this.

# Model specificity is 0.58, meaning that if the passenger did survive is an 58% chance the model will detect this.

# Model FPR is 0.42, meaning that if a passenger did truly survive, there is a 42% chance that the model predicts this passenger as not surviving.

# Model PPV is 0.78, meaning that if a passenger is predicted as not surviving, there is a 78% chance that this passenger indeed not survived.

# Model NPV is 0.82, meaning that if the passenger predicted as surviving, there is a 82% chance that this passenger indeed survived.
```
...
# In our example we have moderate specificity, and very high specificity. High sensitivity means that there are few passengers predicted as surviving when in reality they did not survive. A moderate specificity means that relatively more passengers  were predicted as not surviving when in reality they survived. 

```

The `confusionMatrix()` function from the `caret` package can do a lot of this for us. The function takes two arguments:
